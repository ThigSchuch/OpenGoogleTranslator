    ...
    try:
        ...
        if checkLang(lang):
            ...
            update.message.reply_markdown(f"""Certo! Seu idioma nativo é "{lang}".
                                        \nAgora defina o reverso, vou usar ele quando você digitar em seu Idioma Nativo.
                                        \nExemplo: `/setReverse en`""")
        else:
            update.message.reply_text(f'Não encontrei esse idioma "{lang}", você deve digitar a sigla.\nNão sabe qual? Clique aqui /langs')
    except:
        update.message.reply_mardown('Você deve digitar `/setLang + sigla do idioma.`')
...
def setReverse(update, context):
    try: 
        ...
        if checkLang(lang):
            ...
            update.message.reply_text(f"""Certo! Seu idioma reverso é "{lang}".
                                        \nAgora você já pode mandar qualquer texto para mim.""")
        else:
            update.message.reply_text(f'Não encontrei esse idioma "{lang}", você deve digitar a sigla.\nNão sabe qual? Clique aqui /langs')
    except:
        update.message.reply_markdown('Você deve digitar `/setReverse + sigla do idioma`.')
...
`/setReverse` - Usarei para traduzir quando você digitar em seu Idioma Nativo\n
...
def main():
    updater = Updater(token='1INSIRA SEU TOKEN AQUI', use_context=True)
